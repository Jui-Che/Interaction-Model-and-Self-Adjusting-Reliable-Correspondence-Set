...
bool check_iter_jump(vector<int>& tiny_indices, vector<Point2f>& pick_src, vector<Point2f>& pick_tar) {
    ...
    OptimizedFundamentalMatrixSolver::computeEpipolarErrorFast(pick_src, pick_tar, final_F, v_res);
    if (norm(v_res) < 1)
        return true;
    return false;
}
...
bool checkAllAreas(const vector<Point2f>& tiny_src, const vector<Point2f>& tiny_tar, vector<int>& indices, double area_threshold) {
    for (const auto& pair : COMBINATIONS) {
        if (pair[0] >= indices.size() || pair[1] >= indices.size()) continue;

        int idx0 = indices[pair[0]];
        int idx1 = indices[pair[1]];

        if (idx0 >= tiny_src.size() || idx1 >= tiny_src.size()) continue;
...
void pick_EightCorrespondence(const vector<Point2f>& tiny_src, const vector<Point2f>& tiny_tar, vector<int>& tiny_weight, vector<Point2f>& pick_src, vector<Point2f>& pick_tar, vector<int>& pick_weight) {
    int pick_size = 8, attempt_threshold = 1000, area_threshold = 1000, attempts = 0;  
...
bool tinyCOOSAC(vector<Point2f>& tiny_src, vector<Point2f>& tiny_tar, vector<int>& tiny_indices, pair<int, int> high_idx, vector<int>& tiny_weight, double sigmoid, int QR_times) {
    ...
    Mat tiny_F(3, 3, CV_64F);  

    int no_improvement_count = 0;
    const int max_no_improvement = 1000; 
    double last_best_ratio = 0;
    ...
    while (tiny_confidence < maxConfidence && tiny_iteration < maxIters) {
        ...
        pick_EightCorrespondence(tiny_src, tiny_tar, tiny_weight, pick_src, pick_tar, pick_weight);
        ...
        try {
            tiny_F = OptimizedFundamentalMatrixSolver::solve(pick_src, pick_tar, pick_weight);
            ...
            OptimizedFundamentalMatrixSolver::computeEpipolarErrorFast(tiny_src, tiny_tar, tiny_F, residuals_error);
    ...
    pair<int, int> high_idx, double compact_rate, vector<int>& weight, double sigmoid)
{
    ...
    double global_inlierThreshHold = inlierThresh;
    ...
    while (final_confidence <= maxConfidence && final_iteration < maxIters && QR_times < 10) {
        ...
        tinyWithdraw_size = max(tinyWithdraw_size, 20);
        ...
        if (!best_tiny_F.empty()) {
            OptimizedFundamentalMatrixSolver::computeEpipolarErrorFast(init_src_pts, init_tar_pts, best_tiny_F, global_error_list);

            self_adjusting(global_error_list, global_inlierThreshHold, inlier_count, mask, weight, inliers_outliers_mask,
    ...
    try {
        if (init_src_pts.size() >= 8) {
            Mat eigen_F = OptimizedFundamentalMatrixSolver::solve(init_src_pts, init_tar_pts, weight);

            if (!eigen_F.empty()) {
                vector<double> eigen_error_list(all_size);
                vector<int> eigen_mask(all_size);
                OptimizedFundamentalMatrixSolver::computeEpipolarErrorFast(init_src_pts, init_tar_pts, eigen_F, eigen_error_list);
                ...
                if (eigen_inlierRate > bestfinal_inlierRate) {
                    eigen_F.copyTo(final_F);
        ...
        std::cout << "Warning: Weighted fundamental matrix estimation failed: " << e.what() << std::endl;
    }
